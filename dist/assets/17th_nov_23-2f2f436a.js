import{d as n,c as o,f as s,o as a}from"./index-5a3f0592.js";const i="/assets/17th_nov_23_1-c51677cc.png",c="/assets/17th_nov_23_2-dc596c30.png",r="/assets/17th_nov_23_3-8ef9cae7.png",d="/assets/17th_nov_23_4-1b4b7c14.png",l="/assets/17th_nov_23_5-416b9f7d.png",p={class:"markdown-body"},g={title:"Adding user email to Auth0 token claims",date:"17/11/23"},_="",y=n({__name:"17th_nov_23",setup(h,{expose:t}){return t({frontmatter:{title:"Adding user email to Auth0 token claims",date:"17/11/23"},excerpt:void 0}),(m,e)=>(a(),o("div",p,e[0]||(e[0]=[s('<p>So I managed to find <a href="https://auth0.com/docs/get-started/architecture-scenarios/spa-api/api-implementation-nodejs#4-determine-the-user-identity">this documentation</a> on how to add the user’s details to your token’s claims.</p><p>Just one problem, the documentation is outdated. It mentions using Rules, but when I went to the rules page, this is what I got:</p><p><img src="'+i+'" alt="Auth0 rules is being deprecated"></p><p>Well, no point learning something that is going to be deprecated right.</p><p>So I did a bit more digging and came across <a href="https://community.auth0.com/t/post-login-action-not-adding-email-to-access-token/97804">this thread</a>. I don’t exactly have this problem, but it pointed me in the right direction.</p><p>First, login to the Auth0 dashboard, and go to “Actions” &gt; “Flows”. Select the “Login” flow.</p><p>When you’re in the Login flow, create an action like this:</p><p><img src="'+c+`" alt="Create action"></p><p>Once the Action has been created, add the following code:</p><pre><code class="">exports.onExecutePostLogin = async (event, api) =&gt; {
    if (event.authorization) {
        const namespace = &#39;http://localhost:4000&#39;;
        api.accessToken.setCustomClaim(\`\${namespace}/claims/email\`, event.user.email);
    }
};
</code></pre><p>According to the docs and the community issue opened above, the reason why we are using <code class="">namespace</code> is to avoid conflict with existing claims, so make sure to use something unique.</p><p><img src="`+r+'" alt="Add action code"></p><p>Click on “Deploy”, then go back to the Login flow.</p><p>On the right hand side, you should see your newly created “Add email to Access Token” action. Drag and drop this action to your flow like this:</p><p><img src="'+d+`" alt="Add action to flow"></p><p>Click on “Apply” and thats it, from now on when a user logs in, their email will be automatically added to the token’s claims.</p><p>But I still need to test this by updating my backend project. First, I added the namespace I used earlier to my <code class="">.env</code> file:</p><pre><code class="">// .env
