...
</code></pre><p>Much easier to read and I don’t have to worry about SQL syntax! Knex’s query builder also looks a lot like Laravel’s query builder… But its been awhile since Laravel for me, so I don’t remember these things off the top of my head anymore.</p><p>View the changes I’ve made in this repo: <a href="https://github.com/fattynomnom/itrack-expressjs">itrack-expressjs</a>.</p><p><strong>Next thing I’ll be working on:</strong> Integrating Auth0 with Express</p>`,12),j=[U],E={title:"Writing queries using Knex",date:"8/11/23"},R=i({__name:"8th_nov_23",setup(t,{expose:e}){return e({frontmatter:{title:"Writing queries using Knex",date:"8/11/23"},excerpt:void 0}),(a,r)=>(o(),s("div",T,j))}}),C="/assets/13th_nov_23_1-fdfc503b.png",N="/assets/13th_nov_23_2-cdae4a2d.png",$="/assets/13th_nov_23_3-95325731.png",Q={class:"markdown-body"},M=d('<p>Previously I talked about setting up an Apollo server with ExpressJs for GraphQL APIs, but these APIs aren’t protected. Ideally I want my APIs to be protected by something like an Authorization header.</p><p>I decided to go with <a href="https://auth0.com/docs/">Auth0</a> as this seems like the industry standard for authentication, plus it is a method that I haven’t tried before, so plenty of opportunity for learning.</p><p>First, I followed <a href="https://auth0.com/docs/quickstart/backend/nodejs/interactive">this tutorial</a> and created an API on the Auth0 dashboard like so:</p><p><img src="'+C+`" alt="Create API on Auth0 dashboard"></p><p>Then, using the <code class="">express-oauth2-jwt-bearer</code> package mentioned in the tutorial, I created the middleware that checks for the JWT token in the Authorization header:</p><pre><code class="">// src/index.ts
const checkJwt = auth({
...
</code></pre><p>I finished up by adding my generated types file to my Eslint ignore list as well as my <code class="">.gitignore</code>.</p><p>I’ve committed my changes to <a href="https://github.com/fattynomnom/itrack-expressjs">this repo</a>.</p><p>Ok now onto the next thing: adding user details to token claims.</p>`,18),X=[V],Z={title:"Generate typings from GraphQL schema",date:"16/11/23"},ee=i({__name:"16th_nov_23",setup(t,{expose:e}){return e({frontmatter:{title:"Generate typings from GraphQL schema",date:"16/11/23"},excerpt:void 0}),(a,r)=>(o(),s("div",Y,X))}}),te="/assets/17th_nov_23_1-c51677cc.png",oe="/assets/17th_nov_23_2-dc596c30.png",se="/assets/17th_nov_23_3-8ef9cae7.png",ae="/assets/17th_nov_23_4-1b4b7c14.png",ne="/assets/17th_nov_23_5-416b9f7d.png",re={class:"markdown-body"},ie=d('<p>So I managed to find <a href="https://auth0.com/docs/get-started/architecture-scenarios/spa-api/api-implementation-nodejs#4-determine-the-user-identity">this documentation</a> on how to add the user’s details to your token’s claims.</p><p>Just one problem, the documentation is outdated. It mentions using Rules, but when I went to the rules page, this is what I got:</p><p><img src="'+te+'" alt="Auth0 rules is being deprecated"></p><p>Well, no point learning something that is going to be deprecated right.</p><p>So I did a bit more digging and came across <a href="https://community.auth0.com/t/post-login-action-not-adding-email-to-access-token/97804">this thread</a>. I don’t exactly have this problem, but it pointed me in the right direction.</p><p>First, login to the Auth0 dashboard, and go to “Actions” &gt; “Flows”. Select the “Login” flow.</p><p>When you’re in the Login flow, create an action like this:</p><p><img src="'+oe+`" alt="Create action"></p><p>Once the Action has been created, add the following code:</p><pre><code class="">exports.onExecutePostLogin = async (event, api) =&gt; {
    if (event.authorization) {
        const namespace = &#39;http://localhost:4000&#39;;
        api.accessToken.setCustomClaim(\`\${namespace}/claims/email\`, event.user.email);
    }
};
</code></pre><p>According to the docs and the community issue opened above, the reason why we are using <code class="">namespace</code> is to avoid conflict with existing claims, so make sure to use something unique.</p><p><img src="`+se+'" alt="Add action code"></p><p>Click on “Deploy”, then go back to the Login flow.</p><p>On the right hand side, you should see your newly created “Add email to Access Token” action. Drag and drop this action to your flow like this:</p><p><img src="'+ae+`" alt="Add action to flow"></p><p>Click on “Apply” and thats it, from now on when a user logs in, their email will be automatically added to the token’s claims.</p><p>But I still need to test this by updating my backend project. First, I added the namespace I used earlier to my <code class="">.env</code> file:</p><pre><code class="">// .env
...
ngrok http 4000
</code></pre><p>You should see something like this after:</p><p><img src="`+ue+`" alt="Ngrok terminal"></p><p>Use that URL in your code like so:</p><pre><code class="">exports.onExecutePostLogin = async (event, api) =&gt; {
    if (event.stats.logins_count === 1) {
        await axios.post(
...
</code></pre><p>I’ve tested it, and now whenever someone logs in for the first time (whether by email/password or by social connections), their email would be added to my db.</p><p>Now onto actual development.</p><p><strong>Resources</strong>:</p><ul><li><a href="https://auth0.com/docs/customize/actions/flows-and-triggers/post-user-registration-flow#store-the-auth0-user-id-in-a-remote-system">Store the Auth0 user ID in a remote system</a></li><li><a href="https://community.auth0.com/t/problems-with-post-user-registration-action/101314">Problems with post user registration action</a></li><li><a href="https://ngrok.com/download">Download Ngrok</a></li><li><a href="https://community.auth0.com/t/how-do-i-call-my-api-from-a-rule/41309">How do I call my API from a rule</a></li></ul>`,41),fe=[me],we={title:"Adding registered Auth0 user to database",date:"19/11/23"},ye=i({__name:"19th_nov_23",setup(t,{expose:e}){return e({frontmatter:{title:"Adding registered Auth0 user to database",date:"19/11/23"},excerpt:void 0}),(a,r)=>(o(),s("div",ge,fe))}}),_e={xmlns:"http://www.w3.org/2000/svg",fill:"none",stroke:"currentColor","stroke-width":"1.5",class:"w-6 h-6",viewBox:"0 0 24 24"},be=c("path",{"stroke-linecap":"round","stroke-linejoin":"round",d:"m19.5 8.25-7.5 7.5-7.5-7.5"},null,-1),Ie=[be];function xe(t,e){return o(),s("svg",_e,[...Ie])}const ke={render:xe},ve=t=>(_("data-v-6f12fdb3"),t=t(),b(),t),Ae={class:"divide-y p-10 md:px-28 lg:py-36 h-full lg:overflow-y-auto"},qe=d('<section data-v-6f12fdb3><h1 data-v-6f12fdb3>Hey there!</h1><p data-v-6f12fdb3> I&#39;m a part time software engineer and full time cat person who likes problem solving and puzzles. </p><p data-v-6f12fdb3> I don&#39;t have any fancy repositories for you to see (I&#39;m sure you can understand as a software developer, most of our client works are proprietary), though you can still head over to my <a href="https://github.com/fattynomnom" data-v-6f12fdb3>Github</a> to view my personal projects I&#39;m working on when I have the time! </p><p data-v-6f12fdb3> I&#39;m a huge advocate for Typescript, object-oriented programming and clean coding practices (DRY, SOLID). I know writing it is a pain, but reading clean code is <i data-v-6f12fdb3>*chefs kiss*</i>. Do I sit back sometimes and admire clean code structures? Yes, yes I do. </p><p data-v-6f12fdb3> My hobbies are gardening, reading mangas, watching movies and starting 1001 coding projects that I don&#39;t finish. </p><p data-v-6f12fdb3> I&#39;m trying to hold myself accountable by creating this site to document my learning process. Hopefully I&#39;ll be able to learn and update this space as much as possible (when life, work and cats doesn&#39;t get in the way). </p><p data-v-6f12fdb3> Just in case you&#39;re curious, this site was made with VueJs and TailwindCss (both of which I <i data-v-6f12fdb3>think</i> I&#39;m a master at). </p></section>',1),Le=ve(()=>c("h1",null,"What I'm learning",-1)),Se=["onClick"],Pe={class:"text-sm text-gray-400"},Te=i({__name:"HomeView",setup(t){const e=h([{frontmatter:we,component:ye,displayed:!1},{frontmatter:ce,component:le,displayed:!1},{frontmatter:Z,component:ee,displayed:!1},{frontmatter:K,component:W,displayed:!1},{frontmatter:H,component:O,displayed:!1},{frontmatter:E,component:R,displayed:!1},{frontmatter:S,component:P,displayed:!1}]);return(a,r)=>(o(),s("main",Ae,[qe,c("section",null,[Le,(o(!0),s(u,null,g(e.value,n=>(o(),s("div",{key:n.frontmatter.title,class:"space-y-5 p-5 md:p-10 rounded-lg bg-gray-50 overflow-hidden"},[c("div",{class:"flex items-center justify-between cursor-pointer space-x-5",onClick:je=>n.displayed=!n.displayed},[c("div",null,[c("p",Pe,l(n.frontmatter.date),1),c("h2",null,l(n.frontmatter.title),1)]),p(f(ke),{class:m(["w-5 h-5 text-gray-500",{"rotate-180":n.displayed}])},null,8,["class"])],8,Se),p(y,null,{default:w(()=>[n.displayed?(o(),I(x(n.component),{key:0})):k("",!0)]),_:2},1024)]))),128))])]))}});const Ue=(t,e)=>{const a=t.__vccOpts||t;for(const[r,n]of e)a[r]=n;return a},Re=Ue(Te,[["__scopeId","data-v-6f12fdb3"]]);export{Re as default};
