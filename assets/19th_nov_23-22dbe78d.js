import{d as o,c as n,f as s,o as a}from"./index-5a3f0592.js";const i="/assets/19th_nov_23_1-457c73c9.png",r="/assets/19th_nov_23_3-a64b63b3.png",h="/assets/19th_nov_23_2-a625fc70.png",l={class:"markdown-body"},u={title:"Adding registered Auth0 user to database",date:"19/11/23"},g="",m=o({__name:"19th_nov_23",setup(c,{expose:t}){return t({frontmatter:{title:"Adding registered Auth0 user to database",date:"19/11/23"},excerpt:void 0}),(d,e)=>(a(),n("div",l,e[0]||(e[0]=[s(`<p>So I <em>thought</em> I got my bases all covered but I forgot one thing… When a user registers with Auth0, the user is not being added to my PostgreSQL db, but into Auth0’s db instead.</p><p>Considering they are two separate databases, doing relational queries like “Fetch transactions belonging to specific user”, is harder to do.</p><p>So initially I had followed <a href="https://auth0.com/docs/customize/actions/flows-and-triggers/post-user-registration-flow#store-the-auth0-user-id-in-a-remote-system">this tutorial</a> to add the registered user to my db after user has completed registration flow.</p><p>The tutorials mentions to add an action to the Post Registration flow, something like this:</p><pre><code class="">exports.onExecutePostUserRegistration = async (event) =&gt; {
    await axios.post(&quot;https://my-api.exampleco.com/users&quot;, { params: { email: event.user.email }});
};
</code></pre><p>And it worked… for users who <em>actually</em> went through the registration flow with an email and password. If the user “registers” using their Google account, technically, they would not go through this registration flow (after much manual testing). I wished the docs would’ve been clearer about this.</p><p>So instead, I’m going to follow <a href="https://community.auth0.com/t/problems-with-post-user-registration-action/101314">this suggestion</a> to use the <strong>Post Login flow</strong> instead of the Post Registration flow, by checking the number of logins when user logins. If the number of logins is 1, I will assume that the user has just registered and add their email to my db.</p><p>First, go to “Flows” on the Auth0 dashboard, then select the “Login” flow. Then on the right hand side, create an action for the flow like so:</p><p><img src="`+i+`" alt="Create action"></p><p>For the code I am using this:</p><pre><code class="">const axios = require(&quot;axios&quot;);
...
ngrok http 4000
</code></pre><p>You should see something like this after:</p><p><img src="`+h+`" alt="Ngrok terminal"></p><p>Use that URL in your code like so:</p><pre><code class="">exports.onExecutePostLogin = async (event, api) =&gt; {
    if (event.stats.logins_count === 1) {
        await axios.post(
...
</code></pre><p>I’ve tested it, and now whenever someone logs in for the first time (whether by email/password or by social connections), their email would be added to my db.</p><p>Now onto actual development.</p><p><strong>Resources</strong>:</p><ul><li><a href="https://auth0.com/docs/customize/actions/flows-and-triggers/post-user-registration-flow#store-the-auth0-user-id-in-a-remote-system">Store the Auth0 user ID in a remote system</a></li><li><a href="https://community.auth0.com/t/problems-with-post-user-registration-action/101314">Problems with post user registration action</a></li><li><a href="https://ngrok.com/download">Download Ngrok</a></li><li><a href="https://community.auth0.com/t/how-do-i-call-my-api-from-a-rule/41309">How do I call my API from a rule</a></li></ul>`,41)])))}});export{m as default,g as excerpt,u as frontmatter};
